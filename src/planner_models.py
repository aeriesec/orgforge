...
class EngineerDayPlan:
    ...
    focus_note:    str  = ""      # one-sentence LLM colour — e.g. "Jax is in heads-down mode today"

    @property
    def capacity_hrs(self) -> float:
        """Available hours — shrinks under stress and on-call duty."""
        base = 6.0
        if self.is_on_call:    base -= 1.5
        if self.stress_level > 80: base -= 2.0
        elif self.stress_level > 60: base -= 1.0
        return max(base, 1.5)
    ...
    def apply_incident_pressure(self, incident_title: str, hrs_lost: float = 3.0):
        ...
        defer_order = ["deep_work", "design_doc", "mentoring", "1on1",
                       "async_question", "pr_review", "ticket_progress"]
...
class ProposedEvent:
    ...
    is_novel:     bool = False    # True if LLM proposed an event type not in KNOWN_EVENT_TYPES
...
class OrgDayPlan:
    ...
    def all_events_by_priority(self) -> List[ProposedEvent]:
        ...
        for dept in sorted(self.dept_plans.keys(),
                           key=lambda d: 0 if "eng" in d.lower() else 1):
...
KNOWN_EVENT_TYPES = {
    ...
    "incident_opened", "incident_resolved", "escalation_chain",
    "fix_in_progress", "postmortem_created", "knowledge_gap_detected",
    # Engineering — routine
    "standup", "pr_review", "ticket_progress", "design_discussion",
    "async_question", "code_review_comment", "deep_work_session",
    # Sprint
    "sprint_planned", "retrospective", "sprint_goal_updated",
    # Cross-dept
    "leadership_sync", "feature_request_from_sales", "stability_update_to_sales",
    "hr_checkin", "morale_intervention", "1on1_scheduled",
    # External
