...
WORKER_MODEL = Ollama(
    model=_bare_model(_CFG["models"]["worker"]),
    base_url=_CFG["models"]["base_url"]
)
...
def write_eml(
    ...
    msg["Message-ID"] = message_id or f"<{random.randint(10000,99999)}@{COMPANY_DOMAIN}>"
    if cc_names:
...
class EventLog:
    ...
    def __init__(self, snapshot_path: str = SNAPSHOT_PATH):
        ...
        if os.path.exists(snapshot_path):
            ...
            console.print(f"[yellow]⚠  No snapshot found at {snapshot_path}. "
...
class EmailGen:
    ...
    def run(self):
        console.print("[bold cyan]Email Generator (v3 — event-log driven)[/bold cyan]\n")
    ...
    def _incident_escalation_threads(self):
        ...
        for inc in resolved[:MAX_INCIDENT_THREADS]:
            ...
            prod_members = [n for n in ORG_CHART.get(prod_dept, []) if n != incident_lead]
            ...
            turns = [
                {
                    ...
                    "cc":   [eng_peer],
                    "date": inc_date,
                    "body": _llm_body(on_call, "Open a P1 incident escalation email. Be terse. State what's broken and that you're investigating.", facts_str),
                },
                {
                    "from": incident_lead,
                    "to":   [on_call],
                    ...
                    "date": inc_date,
                    "body": _llm_body(incident_lead, "Reply to the P1 alert. Ask for ETA. Express concern about the sprint and upcoming launch.", facts_str),
                },
                {
                    "from": on_call,
                    "to":   [incident_lead, hr_lead, prod_peer],
                    "date": inc_date,
                    "body": _llm_body(on_call, "Give the root cause update and estimated resolution time. Reference the exact root cause.", facts_str),
                },
                {
                    "from": eng_peer,
                ...
                },
                {
                    ...
                    "date": inc_date,
                    "body": _llm_body(on_call, "Send the all-clear. State that the incident is resolved. Mention postmortem.", facts_str),
                },
            ]
            thread_dir = f"{EMAIL_OUT}/threads/incident_{ticket_id}"
            write_thread(thread_dir, f"[P1] Incident: {ticket_id} — {legacy_name} Failure", turns)
            console.print(f"    [green]✓[/green] {ticket_id} thread ({duration}d, PR {pr_id})")
    ...
    def _sprint_weekly_syncs(self):
        ...
        for sp in sprints:
            ...
            kickoff_body = _llm_body(sender, "Write a sprint kickoff email to the whole team. Mention the sprint goal and ticket list.", facts_str)
            write_eml(
            ...
            mid_date = sp_date
            mid_body = _llm_body(sender, "Write a mid-sprint check-in email to leads. Ask for status, blockers, confidence.", facts_str)
            write_eml(
    ...
    def _leadership_sync_emails(self):
        ...
        for week in range(1, 5):
            ...
            inc_this_wk = [r for r in resolved
                           if r.get("day", 0) <= week * 5]
            inc_summary = (
                ", ".join(
                    f"{r['artifact_ids'].get('jira','?')} ({r['facts'].get('root_cause','?')[:35]})"
                    for r in inc_this_wk[-2:]
                ) if inc_this_wk else "none"
            )
        ...
        console.print(f"    [green]✓[/green] 4 leadership sync emails.")
    ...
    def _knowledge_gap_threads(self):
        ...
        if not gaps:
            bill_incidents = [
                e for e in self.log.by_type("incident_opened")
        ...
        gap      = gaps[0]
        gap_area = gap["facts"].get("gap_area", [_CFG.get("legacy_system", {}).get("name", "legacy system")])
        ...
        turns = [
            {
                "from": on_call, "to": [incident_lead, hr_lead, eng_peer],
                "date": gap_date,
                "body": _llm_body(
                    on_call,
            ...
            {
                "from": incident_lead, "to": [on_call, hr_lead, eng_peer],
                "date": gap_date,
                "body": _llm_body(
                    incident_lead,
            ...
            {
                "from": hr_lead, "to": [incident_lead, on_call, eng_peer],
                "date": gap_date,
                "body": _llm_body(
                    hr_lead,
            ...
            {
                "from": on_call, "to": [hr_lead, incident_lead],
                "date": gap_date,
                "body": _llm_body(
                    on_call,
        ...
        console.print(f"    [green]✓[/green] Knowledge gap thread written ({departed_name} / {gap_area}).")
    ...
    def _hr_morale_emails(self):
        ...
        on_call    = resolve_role("on_call_engineer")
        new_hire   = _CFG["simulation"].get("new_hire") or (ORG_CHART.get(_CFG["simulation"].get("new_hire_dept", "Product"), [""])[0])
        ...
        if avg < intervention_threshold:
            ...
            console.print(f"    [yellow]Morale intervention email written (avg={avg:.2f})[/yellow]")
        ...
        console.print(f"    [green]✓[/green] HR emails written.")
    ...
    def _sales_pipeline_emails(self):
        ...
        resolved   = self.log.resolved_incidents()
        start_date = datetime.strptime(_CFG["simulation"]["start_date"], "%Y-%m-%d")
        accounts   = _CFG.get("sales_accounts", ["Acme Corp", "Beta LLC", "Gamma Inc", "Delta Co"])
        ...
        for week in range(1, 4):
            ...
            stability   = "stable" if not inc_this_wk else f"recovering ({len(inc_this_wk)} incident(s) this period)"
        ...
        console.print(f"    [green]✓[/green] 3 pipeline emails.")
