...
CONFIG = {
    "simulation": {"company_name": "TestCorp", "domain": "testcorp.com",
                   "start_date": "2026-01-01", "max_days": 1},
...
def _make_sprint_context(
    ...
    return SprintContext(
        ...
        capacity_by_member={"Alice": 6.0, "Bob": 6.0} if members is None else {m: 6.0 for m in members},
...
def _make_dept_plan(
    ...
    eng_plans = [
        EngineerDayPlan(
            name=n, dept=dept,
            agenda=[AgendaItem(activity_type="ticket_progress",
                               description="Sprint work", estimated_hrs=3.0)],
            stress_level=25,
        )
        for n in names
    ]
    return DepartmentDayPlan(
        ...
        proposed_events=[ProposedEvent(
            event_type="normal_day_slack", actors=names,
            rationale="Standup", facts_hint={}, priority=2,
        )],
        cross_dept_signals=[],
        planner_reasoning="",
        day=day,
...
class TestDepartmentPlannerParsePlan:
    def test_valid_json_produces_correct_theme(self, dept_planner):
        ...
        plan = dept_planner._parse_plan(raw, "Focus day", 5, "2026-01-05", [])
    ...
    def test_proposed_events_sorted_by_priority(self, dept_planner):
        ...
        data["proposed_events"] = [
            {"event_type": "low_pri",  "actors": ["Alice"], "rationale": "",
             "facts_hint": {}, "priority": 3, "is_novel": False, "artifact_hint": None},
            {"event_type": "high_pri", "actors": ["Bob"],   "rationale": "",
             "facts_hint": {}, "priority": 1, "is_novel": False, "artifact_hint": None},
        ]
        plan = dept_planner._parse_plan(json.dumps(data), "Theme", 5, "2026-01-05", [])
    ...
    def test_invalid_json_returns_fallback_plan(self, dept_planner):
        plan = dept_planner._parse_plan("{not valid json}", "Fallback theme", 5, "2026-01-05", [])
    ...
    def test_markdown_fences_stripped_before_parse(self, dept_planner):
        ...
        plan = dept_planner._parse_plan(raw, "Theme", 5, "2026-01-05", [])
        assert plan.theme == "Steady progress on reliability improvements"
    ...
    def test_invented_name_in_llm_response_is_dropped(self, dept_planner):
        ...
        data["engineer_plans"] = data["engineer_plans"] + [{
            ...
            "agenda": [{"activity_type": "deep_work", "description": "Ghost task",
                        "related_id": None, "collaborator": [], "estimated_hrs": 2.0}],
        }]
        plan = dept_planner._parse_plan(json.dumps(data), "Theme", 5, "2026-01-05", [])
        names = {ep.name for ep in plan.engineer_plans}
    ...
    def test_ownership_violation_stripped_with_sprint_context(self, dept_planner):
        ...
        plan = dept_planner._parse_plan(json.dumps(data), "Theme", 5, "2026-01-05", [], sprint_context=ctx)
    ...
    def test_cross_dept_signals_attached_to_plan(self, dept_planner):
        signals = [CrossDeptSignal(
            source_dept="Sales", event_type="customer_escalation",
            summary="Customer angry", day=4, relevance="direct",
        )]
        plan = dept_planner._parse_plan(
...
class TestDepartmentPlannerFallback:
    ...
    def test_fallback_reasoning_mentions_fallback(self, dept_planner):
        plan = dept_planner._fallback_plan("Theme", 5, "2026-01-05", [])
        assert "Fallback" in plan.planner_reasoning or "fallback" in plan.planner_reasoning.lower()
...
class TestDepartmentPlannerHelpers:
    ...
    def test_build_roster_contains_stress_values(self, dept_planner, mock_graph_dynamics):
        mock_graph_dynamics._stress = {"Alice": 55, "Bob": 10}
        roster = dept_planner._build_roster(mock_graph_dynamics)
        assert "55" in roster
        assert "10" in roster

    def test_dept_history_returns_no_history_string_when_empty(self, dept_planner, mock_mem):
    ...
    def test_dept_history_filters_to_relevant_days(self, dept_planner, mock_mem):
        ...
        old_event = SimEvent(
            type="day_summary", timestamp="2026-01-01T09:00:00",
            day=1, date="2026-01-01", actors=["Alice"],
            artifact_ids={}, facts={"active_actors": ["Alice"],
                                    "system_health": 80, "morale_trend": "stable",
    ...
    def test_format_cross_signals_with_signals(self, dept_planner):
        signals = [CrossDeptSignal(
            source_dept="Sales", event_type="customer_escalation",
            summary="Big client unhappy", day=4, relevance="direct",
        )]
...
class TestExtractCrossSignals:
    def _make_orchestrator(self):
        with patch("day_planner.PlanValidator"), \
    ...
    def test_incident_resolved_creates_signal_for_other_depts(self, mock_mem):
        orch = self._make_orchestrator()
        event = SimEvent(
            type="incident_resolved", timestamp="2026-01-05T10:00:00",
            day=5, date="2026-01-05", actors=["Alice"],
            artifact_ids={}, facts={}, summary="Incident resolved",
        )
    ...
    def test_signal_relevance_direct_within_2_days(self, mock_mem):
        ...
        event = SimEvent(
            type="incident_resolved", timestamp="2026-01-05T10:00:00",
            day=4, date="2026-01-04", actors=["Alice"],
            artifact_ids={}, facts={}, summary="Incident resolved",
        )
        mock_mem.get_event_log.return_value = [event]
        signals = orch._extract_cross_signals(mock_mem, day=5)
    ...
    def test_signal_relevance_indirect_beyond_2_days(self, mock_mem):
        orch = self._make_orchestrator()
        event = SimEvent(
            type="incident_resolved", timestamp="2026-01-01T10:00:00",
            day=2, date="2026-01-02", actors=["Alice"],
            artifact_ids={}, facts={}, summary="Old incident",
        )
        mock_mem.get_event_log.return_value = [event]
        signals = orch._extract_cross_signals(mock_mem, day=5)
        sales_signals = signals.get("Sales", [])
    ...
    def test_irrelevant_event_type_ignored(self, mock_mem):
        orch = self._make_orchestrator()
        event = SimEvent(
            type="some_untracked_event", timestamp="2026-01-05T10:00:00",
            day=5, date="2026-01-05", actors=["Alice"],
            artifact_ids={}, facts={}, summary="Ignored",
        )
        mock_mem.get_event_log.return_value = [event]
        signals = orch._extract_cross_signals(mock_mem, day=5)
        assert signals == {}
    ...
    def test_source_dept_not_included_in_its_own_signals(self, mock_mem):
        ...
        event = SimEvent(
            type="incident_resolved", timestamp="2026-01-05T10:00:00",
            day=5, date="2026-01-05", actors=["Alice"],
            artifact_ids={}, facts={}, summary="eng incident",
        )
        mock_mem.get_event_log.return_value = [event]
        signals = orch._extract_cross_signals(mock_mem, day=5)
...
class TestOrgCoordinatorCoordinate:
    ...
    def test_dept_plans_preserved_in_org_plan(self):
        ...
        state = _make_mock_state()
        with patch("day_planner.Agent"), patch("day_planner.Task"), \
             patch("day_planner.Crew") as mock_crew:
    ...
    def test_org_theme_preserved_in_org_plan(self):
        ...
        with patch("day_planner.Agent"), patch("day_planner.Task"), \
             patch("day_planner.Crew") as mock_crew:
            mock_crew.return_value.kickoff.return_value = raw
            org_plan = coord.coordinate(dept_plans, state, 5, "2026-01-05", "My Org Theme")
...
class TestDepartmentPlannerPlan:
    def test_plan_returns_department_day_plan(self, dept_planner, mock_graph_dynamics, mock_mem):
        state = _make_mock_state()
        raw   = json.dumps(VALID_PLAN_JSON)
        with patch("day_planner.Agent"), patch("day_planner.Task"), \
             patch("day_planner.Crew") as mock_crew:
            ...
            plan = dept_planner.plan(
                org_theme="Theme", day=5, date="2026-01-05",
                state=state, mem=mock_mem, graph_dynamics=mock_graph_dynamics,
                cross_signals=[], sprint_context=_make_sprint_context(),
            )
        assert isinstance(plan, DepartmentDayPlan)
        assert plan.dept == "Engineering"

    def test_plan_falls_back_on_llm_json_error(self, dept_planner, mock_graph_dynamics, mock_mem):
        state = _make_mock_state()
        with patch("day_planner.Agent"), patch("day_planner.Task"), \
             patch("day_planner.Crew") as mock_crew:
    ...
    def test_morale_label_low_when_morale_below_45(self, dept_planner, mock_graph_dynamics, mock_mem):
        """
        ...
        with patch("day_planner.Agent"), patch("day_planner.Task"), \
            ...
            mock_crew.return_value.kickoff.return_value = raw
            plan = dept_planner.plan(
                org_theme="Crisis", day=5, date="2026-01-05",
                state=state, mem=mock_mem, graph_dynamics=mock_graph_dynamics,
                cross_signals=[],
            )
...
class TestDayPlannerOrchestratorInit:
    ...
    def test_external_contacts_included_in_validator(self):
        ...
        with patch("day_planner.PlanValidator") as mock_validator_cls, \
             patch("day_planner.OrgCoordinator"):
            DayPlannerOrchestrator(cfg, MagicMock(), MagicMock())
        call_kwargs = mock_validator_cls.call_args
        ext_names = call_kwargs.kwargs.get("external_contact_names") or call_kwargs.args[1]
...
class TestDayPlannerOrchestratorPlan:
    ...
    def test_plan_returns_org_day_plan(self, mock_mem, mock_graph_dynamics):
        with patch("day_planner.PlanValidator"), \
             ...
             patch.object(DayPlannerOrchestrator, "_generate_org_theme", return_value="Daily theme"), \
             patch.object(DayPlannerOrchestrator, "_extract_cross_signals", return_value={}), \
             patch.object(DayPlannerOrchestrator, "_recent_day_summaries", return_value=[]):
            orch = DayPlannerOrchestrator(CONFIG, MagicMock(), MagicMock())
            ...
            org_plan  = OrgDayPlan(
                ...
                collision_events=[], coordinator_reasoning="", day=5, date="2026-01-05",
            )
            ...
            result = orch.plan(state=state, mem=mock_mem,
                               graph_dynamics=mock_graph_dynamics, clock=clock)
    ...
    def test_plan_seeds_ticket_actors_today(self, mock_mem, mock_graph_dynamics):
        ...
        with patch("day_planner.PlanValidator"), \
             patch("day_planner.OrgCoordinator") as mock_coord_cls, \
             patch("day_planner.TicketAssigner") as mock_ta_cls, \
             patch.object(DayPlannerOrchestrator, "_generate_org_theme", return_value="Theme"), \
             patch.object(DayPlannerOrchestrator, "_extract_cross_signals", return_value={}), \
             patch.object(DayPlannerOrchestrator, "_recent_day_summaries", return_value=[]):
            orch = DayPlannerOrchestrator(CONFIG, MagicMock(), MagicMock())
            ...
            state = _make_mock_state()
            clock = self._make_clock(state)
            orch.plan(state=state, mem=mock_mem,
                      graph_dynamics=mock_graph_dynamics, clock=clock)

        assert "ENG-101" in state.ticket_actors_today
        assert "Alice" in state.ticket_actors_today["ENG-101"]

    def test_plan_rejected_events_logged_as_sim_events(self, mock_mem, mock_graph_dynamics):
        """Rejected proposed events must be persisted as proposed_event_rejected SimEvents."""
        rejected_event = ProposedEvent(
            event_type="scope_creep", actors=["Dave", "Alice"],
            rationale="Out of scope", facts_hint={}, priority=2,
        )
    ...
    def test_plan_novel_events_logged_as_sim_events(self, mock_mem, mock_graph_dynamics):
        """Novel proposed events must be persisted as novel_event_proposed SimEvents."""
        novel_event = ProposedEvent(
            event_type="new_thing", actors=["Alice"],
            rationale="A genuinely new workflow", facts_hint={},
            priority=2, is_novel=True, artifact_hint="slack",
        )
        with patch("day_planner.PlanValidator"), \
             patch("day_planner.OrgCoordinator") as mock_coord_cls, \
             patch("day_planner.TicketAssigner") as mock_ta_cls, \
