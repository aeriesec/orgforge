...
def aggregate(per_question: List[dict]) -> dict:
    ...
    def _agg(rows):
        ...
        score = [r["scores"]["answer_score"] for r in rows if r["scores"]["answer_score"] is not None]
        corr  = [r["scores"]["correct"] for r in rows if r["scores"]["correct"] is not None]
        return {
            "n":            len(rows),
            "mrr_at_10":    _mean(mrr),
...
def _print_summary(summary: dict, run_id: str) -> None:
    ...
    print(f"{'='*64}")
    print(f"  {'Type':<18} {'MRR@10':>8} {'Recall@10':>10} {'Score':>8} {'Acc':>6} {'N':>4}")
    print(f"  {'-'*58}")
    print(f"  {'OVERALL':<18} {_f(overall.get('mrr_at_10')):>8} "
    ...
    for qtype, m in sorted(summary.get("by_type", {}).items()):
        print(f"  {qtype:<18} {_f(m.get('mrr_at_10')):>8} "
...
def rescore(args: argparse.Namespace) -> None:
    ...
    for entry in per_question:
        ...
        entry["scores"]["correct"] = (new_score >= 0.9) if new_score is not None else None
...
def _update_leaderboard(run_id: str, run_dir: Path, summary: dict, leaderboard_dir: Path) -> None:
    import csv
    from datetime import datetime, timezone
    ...
    _QTYPES = ["CAUSAL", "ESCALATION", "GAP_DETECTION", "PLAN",
               "RETRIEVAL", "ROUTING", "TEMPORAL"]

    def _f(v): return "" if v is None else v

    fieldnames = ["run_id", "timestamp", "tier", "retriever", "generator",
                  "n", "mrr_at_10", "recall_at_10", "answer_score", "accuracy"]
    for qt in _QTYPES:
    ...
    with open(lb_csv, "w", newline="") as f:
        ...
        for row in leaderboard:
            flat = {k: _f(row.get(k)) for k in
                    ["run_id","timestamp","tier","retriever","generator",
                     "n","mrr_at_10","recall_at_10","answer_score","accuracy"]}
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Re-score existing eval results with updated scorer.py")
    ...
    p.add_argument("--leaderboard", default=None,   metavar="DIR",
